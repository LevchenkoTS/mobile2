...
class ChangeTextTest {
    ...
    private val textHomeWork = "Второе задание"
...
//    }
    ...
    @Test
    fun testTryingToSetAnEmptyString() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        ...
        val result = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertNotEquals(result, textToSet)
    }

    @Test
    fun testCheckTextInNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = textHomeWork
        device.findObject(By.res(packageName, "buttonActivity")).click()

        val result1 = device.findObject(
            UiSelector().textContains(textHomeWork)).text
        assertEquals(result1, textHomeWork)
    }
